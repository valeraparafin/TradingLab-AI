...
2. **Switch RR 2 → RR 3.** Almost every out-of-sample-robust cell uses a 1:3 stop/target
   (SL 2% / TP 6%), not the live 1:2. "Let winners run" generalizes across symbols (§5, §6).
...
4. **Concentrate on XLM (best SMC market), then ETH/SOL/XRP/BTC, on 1H; XLM also works on 15m.**
   Watch REVERSAL on BTC 4H — a genuine secondary pocket (§6).
...
**Level 1 — cheap config win (no code):** retune toward the robust set — **RR 2 → RR 3**,
**drop LTC**, focus **XLM + 1H** (and XLM 15m). This moves live trading onto the cells that
actually survive out-of-sample. Expect modest returns (the edge is thin).

**Level 2 — real growth (mini R&D, recommended):** thicken the edge with **entry filters** —
take the SMC signal only when aligned with higher-timeframe trend / sufficient volume / active
sessions. Goal: lift WR from ~33% and PF from ~1.1 into confidently-tradeable territory. Run
through brainstorm → plan → TDD. This is the highest-value direction.

**Also (separate, touches live — needs explicit per-action confirmation):** stop the
clearly-losing live bots (5m, 4H) and remove LTC from watchlists.

...
// try/finally so the real market_data.db file handle is always released, even if an
// assertion throws mid-test (matters on Windows where the .db file lock is enforced).
try {
  ...
  //    (conservative may legitimately produce 0 trades on this dataset; buildReport's
  //    nullish-safe sort keeps the ordering valid, so the assertion stays meaningful.)
  {
